...
public class JobAdminAuthRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        //状态 >> 已停用
        if (ScheduledUserStatusEnum.DISABLE.getValue().equals(userInfo.getUserStatus())) {
            throw new DisabledAccountException();
        }
        //状态 >> 锁定
        if (ScheduledUserStatusEnum.LOCK.getValue().equals(userInfo.getUserStatus())) {
        ...
        //缓存权限
        JobAdminShiroOperation.setPermissions(permissionSet);
        ...
        log.info("[登录认证] 结束 >> 当前用户 result : {} , sessionId : {}", JSON.toJSONString(result), JobAdminShiroOperation.getSessionId());
